...
const AgentTreeNode: React.FC<AgentTreeNodeProps> = ({
  ...
  const matchesSearch = useMemo(() => {
    ...
    return (
      name.includes(query) || agentId.includes(query) || mobile.includes(query)
    );
  }, [searchQuery, agent]);
  ...
  const hasMatchingChildren = useMemo(() => {
    ...
    const checkChildren = (children: SubAgent[]): boolean => {
      return children.some((child) => {
        ...
        return (
          childMatches || (child.children && checkChildren(child.children))
        );
      });
    };
  ...
  const getName = () =>
    [agent.fname, agent.mname, agent.lname].filter(Boolean).join(" ") ||
    agent.name ||
    "Unknown";
    ...
    <div className={styles.treeNode}>
      ...
      >
        ...
        <div
          ...
          title="Click to open Agent Dashboard"
        >
          ...
          <div className={styles.agentDetails}>
            <span className={styles.agentId}>
              ID:{" "}
              {searchQuery
                ? highlightText(agent.agent_id, searchQuery)
                : agent.agent_id}
            </span>
            ...
            <span className={styles.mobile}>
              Mobile:{" "}
              {searchQuery
                ? highlightText(String(agent.mob_num), searchQuery)
                : agent.mob_num}
            </span>
            {agent.email && (
              <>
...
const convertRoleTreeToSubAgent = (
  roleNode: RoleTreeNode,
  depth: number = 0
): SubAgent => {
  ...
  return {
    ...
    status: "active",
    children:
      roleNode.children && Array.isArray(roleNode.children)
        ? roleNode.children.map((child) =>
            convertRoleTreeToSubAgent(child, depth + 1)
          )
        : [],
  };
};
...
export const SubAgents: React.FC = () => {
  ...
  const agentNameOverride = searchParams.get("agentName");
  ...
  const handleAgentClick = (agent: SubAgent) => {
    ...
    if (agent.email) params.set("agentEmail", agent.email);
  ...
  useEffect(() => {
    if (isAuthenticated) fetchSubAgents();
  }, [isAuthenticated, selectedRole, agentIdOverride]);
    ...
    try {
      if (selectedRole) {
        ...
        if (
          ...
          setSubAgents(
            response.response.tree.map((treeNode) =>
        ...
        <div className={styles.pageHeader}>
          <div>
            <h1 className={styles.pageTitle}>
              Sub Agents
              {agentNameOverride && (
                <span className={styles.agentContext}>
                  ({agentNameOverride})
                </span>
              )}
            </h1>
            {agentNameOverride && (
              <p className={styles.roleIndicator} style={{ marginTop: "4px" }}>
                Viewing sub-agents for: {agentNameOverride} (ID:{" "}
                {agentIdOverride})
              </p>
            )}
            {selectedRole ? (
