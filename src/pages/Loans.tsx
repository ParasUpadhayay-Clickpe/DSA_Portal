...
const LENDER_OPTIONS = [{ label: "SPCBL", value: "SPCBL" }];
...
export const Loans: React.FC = () => {
  ...
  const [availableAgents, setAvailableAgents] = useState<
    Array<{ role_id: string; agent_name: string; agent_id: string }>
  >([]);
    ...
    const fetchSubAgents = async () => {
      ...
      try {
        ...
        if (response.response && response.response.length > 0) {
          const flattenAgents = (
            agents: typeof response.response
          ): Array<{ label: string; value: string }> => {
            const result: Array<{ label: string; value: string }> = [];
            agents.forEach((agent) => {
              const name = [agent.fname, agent.mname, agent.lname]
                .filter(Boolean)
                .join(" ");
              result.push({
                label: name || agent.agent_id,
  ...
  }, [isAuthenticated, agentIdOverride]);
    ...
    const fetchAvailableAgents = async () => {
      ...
      try {
        ...
        if (response.status === "Success" && response.response) {
          const flattenAgents = (
            nodes: typeof response.response.tree
          ): Array<{
            role_id: string;
            agent_name: string;
            agent_id: string;
    ...
    try {
      ...
      if (selectedRole) {
        ...
        const entityRequest = {
          ...
          filters: {
            sub_status:
              Array.isArray(filters.sub_status) && filters.sub_status.length > 0
            ...
            loan_status:
              ...
              filters.loan_status.length > 0
                ...
                : undefined,
            user_mob_num:
              filters.search_type === "mob_num" && filters.search_text
                ? (filters.search_text as string)
                : undefined,
            user_id:
              filters.search_type === "user_id" && filters.search_text
                ? (filters.search_text as string)
                : undefined,
          },
          ranges: {
          ...
          text:
            filters.search_type === "name" && filters.search_text
              ? (filters.search_text as string)
              : undefined,
          search_columns: undefined,
        };

        const entityResponse = await entityRoleMappingApi.getAgentEntities(
          entityRequest
        );

        if (entityResponse.status === "Success" && entityResponse.response) {
          const convertedLoans: Loan[] = entityResponse.response.response.map(
            (entity: AgentEntity) =>
              ({
          ...
          setTotal(
            entityResponse.response.pagination?.total_records ||
              convertedLoans.length
          );
        } else {
        ...
        const request = {
          ...
          filters: {
            loan_id:
              searchType === "loan_id" && searchText ? searchText : undefined,
            sub_status:
              Array.isArray(filters.sub_status) && filters.sub_status.length > 0
                ? (filters.sub_status as string[])
                : undefined,
            ...
            lender_id: filters.lender_id
              ? (filters.lender_id as string)
              : undefined,
            mob_num:
              searchType === "mob_num" && searchText ? searchText : undefined,
            "l.user_id":
              searchType === "user_id" && searchText ? searchText : undefined,
          },
  ...
  const [changingAssignment, setChangingAssignment] = useState<string | null>(
    null
  );
  ...
  const handleChangeAssignment = async (
    loanId: string,
    currentAssignedRoleId: string | undefined
  ) => {
    ...
    try {
      const response = await entityRoleMappingApi.changeEntityMapping({
        ...
        entity_type: "loan_id",
        role_type: selectedRole.role_type,
        assigned_by: selectedRole.role_id,
        assigned_to: newAssignedAgent,
      });
      ...
      setError(
        err instanceof Error ? err.message : "Failed to change assignment"
      );
      setChangingAssignment(null);
    }
  };
    ...
    const exportColumns: ExportColumn<Loan>[] = allColumns.map((col) => {
      ...
      return {
        ...
        render: (value: unknown, row: Loan) => {
          ...
          if (
            col.key === "requested_amt" ||
            col.key === "lender_approved_amt" ||
            col.key === "disbursed_amt"
          )
  ...
  const columns: Column<Loan>[] = [
    { key: "loan_id", label: "Loan ID", sortable: true, defaultVisible: true },
    {
      key: "name",
      label: "Name",
      sortable: true,
    ...
    },
    {
      ...
      render: (value, row) =>
        value
          ? String(value)
          : subAgentOptions.find((a) => a.value === row.agent_id)?.label || "-",
    },
    {
      key: "loan_type",
      label: "Loan Type",
      ...
      ? [
          {
            ...
            render: (value, row): ReactNode => {
              ...
              const assignedAgent = availableAgents.find(
                (a) => a.role_id === currentAssignedRoleId
              );
              return (
                <div
  ...
  const dynamicColumns = useMemo(() => {
    ...
    return remainingKeys.map((key) => {
      ...
      if (
        ...
        key === "income" ||
        key === "total_pre_disbursal_charges"
      )
        render = (value) =>
          value ? `₹${Number(value).toLocaleString()}` : "-";
      else if (
        key.toString().includes("percentage") ||
        key.toString().includes("rate") ||
        key.toString().includes("apr")
      )
      ...
      <div className={styles.pageContainer}>
        ...
        <DataTable
          ...
          onRowClick={(row) => {
            ...
            navigate(
              `/loans/${row.loan_id}${queryString ? `?${queryString}` : ""}`,
              { state: { loan: row } }
            );
          }}
